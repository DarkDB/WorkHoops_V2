    ...
    if (userRole === 'club' || userRole === 'agencia') {
      ...
      opportunities.forEach(opp => {
        opp.applications.forEach(app => {
          // Map Spanish enum values to English keys (synced with DB enum)
          if (app.state === 'enviada' || app.state === 'en_revision') {
            applicationsByState.pending++
          } else if (app.state === 'finalizada') {
      ...
      const pendingApplications = applications.filter(a => a.state === 'enviada' || a.state === 'en_revision').length
      ...
      const viewedApplications = applications.filter(a => a.state === 'finalizada').length
