...
import { z } from 'zod'
import { calculateCoachProfileCompletion } from '@/lib/profile-completion'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const profileCompletionPercentage = calculateCoachProfileCompletion({
      fullName: validatedData.fullName,
      city: validatedData.city,
      currentLevel: validatedData.currentLevel,
      totalExperience: validatedData.totalExperience ?? null,
      currentGoal: validatedData.currentGoal,
      ...
      if (profileCompletionPercentage === 100 && existingProfile.profileCompletionPercentage < 100) {
        try {
          ...
          if (user) {
            ...
            console.log('[COACH PROFILE] Profile completed email sent to:', user.email)
          }
        } catch (emailError) {
          console.error('[COACH PROFILE] Failed to send profile completed email:', emailError)
        }
      }
